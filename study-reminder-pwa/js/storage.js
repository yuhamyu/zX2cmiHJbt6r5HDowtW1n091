...
const StorageManager = {
  ...
  async init() {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        // v3: Exams store (multiple exams support)
        if (!db.objectStoreNames.contains(this.STORES.EXAMS)) {
          const examStore = db.createObjectStore(this.STORES.EXAMS, { keyPath: 'id' });
  ...
  async exportData() {
    ...
    const exams = await this.load(this.STORES.EXAMS, []);
  ...
  async importData(data) {
    ...
    if (data.exams) await this.save(this.STORES.EXAMS, data.exams);
    return true;
  },
