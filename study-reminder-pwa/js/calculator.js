...
const Calculator = {
  ...
  /**
   * Calculate daily allocation for a specific exam
   * Uses exam-specific subject weights if available
   * @param {Object} exam - Exam object
   * @param {Array} subjects - All subjects
   * @param {number} dailyHours - Total study hours for the day
   * @returns {Object} - {subjectId: minutes}
   */
  calculateExamAllocation(exam, subjects, dailyHours) {
    if (!exam || !exam.subjectIds || exam.subjectIds.length === 0) {
      return this.calculateDailyAllocation(subjects, dailyHours);
    }

    // Get only subjects for this exam
    const examSubjects = subjects.filter(s => exam.subjectIds.includes(s.id));
    if (examSubjects.length === 0) {
      return this.calculateDailyAllocation(subjects, dailyHours);
    }

    // Calculate weights (use exam-specific or default)
    const totalWeight = examSubjects.reduce((sum, s) => {
      const weight = exam.subjectWeights?.[s.id] ?? s.weight ?? 1;
      return sum + weight;
    }, 0);
    ...
    examSubjects.forEach(subject => {
      const weight = exam.subjectWeights?.[subject.id] ?? subject.weight ?? 1;
