...
├── exams.html          # テスト一覧画面（追加/編集/削除）
├── exam-edit.html      # テスト追加・編集フォーム
├── input.html          # 勉強記録入力（レガシー、ナビから除外）
├── progress.html       # 進捗確認（統計、教科別、週間サマリー）
├── settings.html       # 設定・バックアップ・リセット・テスト管理
...
| exam.js | `ExamManager` | 複数テスト管理・ステータス計算 |
| migration.js | `MigrationManager` | DBマイグレーション |
| app.js | `App` | 統合・初期化・UI連携 |

### 依存関係
```
HTML → storage.js → calculator.js → cat.js → exam.js → migration.js → timer.js → app.js → sw.js
```
...
```javascript
{
  id: 'uuid',
  name: '2学期中間テスト',
  type: 'regular',                    // 'regular' | 'mock' | 'entrance'
  dates: ['2026-05-20', '2026-05-21'], // 1〜3日
  dateUndecided: false,
  subjectIds: ['uuid1', 'uuid2'],     // 対象教科
  subjectWeights: { 'uuid1': 7 },     // テスト別の重み（設定時のみ）
  reviewDays: 3,                       // 復習期間（試験何日前から）
  status: 'upcoming',                  // upcoming | review | completed
  createdAt: 'ISO8601',
  updatedAt: 'ISO8601'
}
```
...
### ExamManager API
- `createExam(data)` - 新規exam作成
- `calculateStatus(exam)` - ステータス判定（upcoming/review/completed）
- `getDaysUntilExam(exam)` - 試験までの日数
- `getDaysUntilReview(exam)` - 復習期間開始までの日数
- `getReviewDaysRemaining(exam)` - 復習期間の残り日数
- `getNextExam(exams)` - 次の試験を取得
- `getActiveExams(exams)` - アクティブな試験一覧
- `formatDateRange(exam)` - 日付範囲の表示形式
- `validate(exam)` - バリデーション
...
### 復習期間の仕組み
1. 試験日の`reviewDays`日前から復習期間開始
2. ホーム画面に「復習期間まであとX日」→「復習期間！残りX日」と表示
3. 試験日を過ぎると自動的に`completed`ステータスに

### マイグレーション
既存ユーザーは`config.examDate`から自動的にexamsストアにデータ移行
